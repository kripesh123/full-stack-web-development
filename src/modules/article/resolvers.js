...
export async function getBySlug(parentValue, { slug }) {
    ...
    if (!article) {
        ...
        throw new Error('The article you are looking for does not exists or has been discontinued.')
    } else {
        return article;
    }
}
...
// Create Article
export async function create(parentValue, { title, slug, description, body, image, type, featured, isActive }, { auth }) {
...
// Update Article
export async function update(parentValue, { id, title, slug, description, body, image, type, featured, isActive, userId }, { auth }) {
    if (auth.isAuthenticated && auth.isCreator || auth.isAuditor || auth.isAdmin) {
